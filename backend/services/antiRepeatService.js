import { db } from '../database.js';

/**
 * Get performance statistics for a DJ.
 * @param {number|string} djId
 * @returns {{ totalPerformances: number, performancesLast30Days: number, roomBreakdown: Array }}
 */
function getDJStats(djId) {
  const id = Number(djId);

  const totalRow = db
    .prepare('SELECT COUNT(*) as cnt FROM dj_performance_log WHERE dj_id = ?')
    .get(id);

  const last30Row = db
    .prepare(
      `SELECT COUNT(*) as cnt FROM dj_performance_log
       WHERE dj_id = ? AND performed_at >= datetime('now', '-30 days')`
    )
    .get(id);

  const roomBreakdown = db
    .prepare(
      `SELECT r.id as room_id, r.name as room_name, COUNT(*) as count
       FROM dj_performance_log dpl
       LEFT JOIN rooms r ON r.id = dpl.room_id
       WHERE dpl.dj_id = ?
       GROUP BY dpl.room_id
       ORDER BY count DESC`
    )
    .all(id);

  return {
    totalPerformances: totalRow ? totalRow.cnt : 0,
    performancesLast30Days: last30Row ? last30Row.cnt : 0,
    roomBreakdown,
  };
}

/**
 * Assess repeat-booking risk for a DJ in a given room.
 * @param {number|string} djId
 * @param {number|string} roomId
 * @returns {{ risk: 'none'|'warning'|'danger', message: string, totalCount: number, roomCount: number }}
 */
function checkRepeatRisk(djId, roomId) {
  const id = Number(djId);
  const rid = roomId ? Number(roomId) : null;

  const totalRow = db
    .prepare('SELECT COUNT(*) as cnt FROM dj_performance_log WHERE dj_id = ?')
    .get(id);
  const totalCount = totalRow ? totalRow.cnt : 0;

  let roomCount = 0;
  if (rid) {
    const roomRow = db
      .prepare(
        'SELECT COUNT(*) as cnt FROM dj_performance_log WHERE dj_id = ? AND room_id = ?'
      )
      .get(id, rid);
    roomCount = roomRow ? roomRow.cnt : 0;
  }

  let risk = 'none';
  let message = 'Aucun risque de répétition détecté.';

  if (totalCount > 10 || roomCount > 6) {
    risk = 'danger';
    message = `Danger : ce DJ a joué ${totalCount} fois au total et ${roomCount} fois dans cette salle. Forte surexposition.`;
  } else if (totalCount > 5 || roomCount > 3) {
    risk = 'warning';
    message = `Attention : ce DJ a joué ${totalCount} fois au total et ${roomCount} fois dans cette salle. Rotation à surveiller.`;
  }

  return { risk, message, totalCount, roomCount };
}

export { getDJStats, checkRepeatRisk };
